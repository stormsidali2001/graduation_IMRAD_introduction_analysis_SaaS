...
import { PredictionOutputDto, PredictionOutputDtoType } from "../validation/PredictionDto";

export const makePrediction = async(sentences:string)=>{

    const movesPredictions:PredictionOutputDtoType = (await getMoves(sentences)).map((prediction,index)=>{
